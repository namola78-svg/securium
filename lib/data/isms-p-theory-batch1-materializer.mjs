...
import { getIsmsPTheoryRegistryRecords } from "./isms-p-theory-registry-segments.mjs";

/** @typedef {{humanSource: string, sme: string, example: string, examPoint: string}} RegistryApproval */
/** @typedef {{lessonsJsonSha256: string, approvedPreviewBodySha256: string, approvedPreviewSummarySha256: string}} RegistryProvenance */
/** @typedef {{official: boolean, title: string, version: string, tier: string, pages: {start: number, end: number}}} RegistrySource */
/** @typedef {{officialCode: string, sourceLessonId: string, approval: RegistryApproval, source: RegistrySource, currentness: string, provenance: RegistryProvenance}} RegistryMetadata */
/** @typedef {{id: string, slug: string, canonicalKey: string, title: string, summary: string, body: string, bodyFormat: string, learningObjectivesJson: string, coreConceptsJson: string, practicalExamplesJson: string, diagramsJson: string, mediaJson: string, version: string, status: string}} RegistryContent */
/** @typedef {{id: string, courseId: string, curriculumNodeId: string, lessonId: string, displayTitle: string, sortOrder: number, difficulty: string, importance?: number, estimatedMinutes: number, isRequired: boolean, unlockCondition: string, completionRule: string, status: string}} RegistryCourseLesson */
/** @typedef {{courseLessonId: string, learningObjectivesOverrideJson: string, additionalBody: string, examPointsJson: string, practicalNotes: string, legalNotes: string, standardNotes: string, evidenceNotes: string, commonMistakes: string, instructorNotes: string, version: string, status: string}} RegistryExtension */
/** @typedef {{content: RegistryContent, courseLesson: RegistryCourseLesson, extension: RegistryExtension, metadata: RegistryMetadata}} RegistryRecord */
/** @typedef {{batchId: string, records: RegistryRecord[]}} RegistrySegment */
/** @typedef {Record<string, string | number | null>} MaterializationRow */
/** @typedef {{id: string, slug: string, canonical_key: string, title: string, summary: string, body: string, body_format: string, learning_objectives_json: string, core_concepts_json: string, practical_examples_json: string, diagrams_json: string, media_json: string, version: string, status: string, created_by: string | null, deleted_at: string | null}} MaterializationContent */
/** @typedef {{id: string, course_id: string, curriculum_node_id: string | null, content_id: string, lesson_id: string | null, display_title: string, sort_order: number, difficulty: string | null, importance: number | null, estimated_minutes: number, is_required: number, unlock_condition: string | null, completion_rule: string, status: string, deleted_at: string | null}} MaterializationCourseLesson */
/** @typedef {{id: string, course_lesson_id: string, learning_objectives_override_json: string | null, additional_body: string | null, exam_points_json: string, practical_notes: string, legal_notes: string, standard_notes: string, evidence_notes: string, common_mistakes: string, instructor_notes: string, version: string, status: string}} MaterializationExtension */
/** @typedef {{body: string, summary: string, example: string, examPoint: string, content: string, courseLesson: string, extension: string, materialization: string}} MaterializationHashes */
/** @typedef {{sourceLessonId: string, source: RegistrySource, currentness: string, repositoryPreviewBodyHash: string, repositoryPreviewSummaryHash: string}} MaterializationTrace */
/** @typedef {{code: string, content: MaterializationContent, courseLesson: MaterializationCourseLesson, extension: MaterializationExtension, hashes: MaterializationHashes, trace: MaterializationTrace}} MaterializationManifestEntry */
/** @typedef {{entity: "content" | "courseLesson" | "extension", code: string, plannedId: string, existingId: string | null, classification: "CREATE" | "NOOP" | "CONFLICT", payloadHash: string, reason: string, operationId: string}} MaterializationOperation */
/** @typedef {{CREATE: number, NOOP: number, CONFLICT: number}} MaterializationCounts */
/** @typedef {{course: {valid: boolean, rowCount: number}, approvedCount: number, holdCount: number, holdOperationCount: number, operationSlots: number, expectedOperationSlots: number, historicalBatch: boolean, operations: MaterializationOperation[], counts: MaterializationCounts, conflictGate: "PASS" | "FAIL"}} MaterializationPlan */
...
export function buildIsmsPBatch1MaterializationManifest() {
  ...
  return buildMaterializationManifest(records);
}

/**
 * Recomputes the registry's lessonsJsonSha256 over the exact source JSON
 * bytes. The source hash is a file/content hash, so callers must not parse
 * and re-serialize the document before calling this function.
 *
 * @param {string | Uint8Array} sourceLessonsJson
 * @returns {string}
 */
export function computeIsmsPSourceLessonsJsonSha256(sourceLessonsJson) {
  if (
    typeof sourceLessonsJson !== "string" &&
    !(sourceLessonsJson instanceof Uint8Array)
  ) {
    throw new TypeError(
      "ISMS_P_SOURCE_LESSONS_JSON_MUST_BE_UTF8_TEXT_OR_BYTES",
    );
  }
  return sha256(sourceLessonsJson);
}

/**
 * Compares a source document with an expected lessonsJsonSha256 and verifies
 * that the requested source lesson identity occurs exactly once. This helper
 * is comparison-only: its inputs do not establish source authority.
 *
 * @param {{sourceLessonId: string, expectedLessonsJsonSha256: string, sourceLessonsJson: string | Uint8Array | null}} input
 * @returns {{sourceLessonId: string, expectedLessonsJsonSha256: string, actualLessonsJsonSha256: string | null, status: "VERIFIED" | "MISSING" | "MISMATCH" | "IDENTITY_MISMATCH", reason: string}}
 */
export function verifyIsmsPSourceLessonsJsonHash(input) {
  if (
    typeof input?.sourceLessonId !== "string" ||
    input.sourceLessonId.length === 0 ||
    typeof input.expectedLessonsJsonSha256 !== "string" ||
    !/^[a-f0-9]{64}$/.test(input.expectedLessonsJsonSha256)
  ) {
    throw new TypeError("ISMS_P_SOURCE_LESSON_HASH_INPUT_INVALID");
  }
  if (input.sourceLessonsJson === null) {
    return {
      sourceLessonId: input.sourceLessonId,
      expectedLessonsJsonSha256: input.expectedLessonsJsonSha256,
      actualLessonsJsonSha256: null,
      status: "MISSING",
      reason: "SOURCE_LESSONS_JSON_NOT_PROVIDED",
    };
  }
  const actualLessonsJsonSha256 = computeIsmsPSourceLessonsJsonSha256(
    input.sourceLessonsJson,
  );
  const sourceText =
    typeof input.sourceLessonsJson === "string"
      ? input.sourceLessonsJson
      : decodeUtf8(input.sourceLessonsJson);
  if (sourceText === null) {
    return {
      sourceLessonId: input.sourceLessonId,
      expectedLessonsJsonSha256: input.expectedLessonsJsonSha256,
      actualLessonsJsonSha256,
      status: "IDENTITY_MISMATCH",
      reason: "SOURCE_LESSONS_JSON_IS_NOT_VALID_UTF8",
    };
  }
  let parsed;
  try {
    ...
    return {
      ...
      actualLessonsJsonSha256,
      status: "IDENTITY_MISMATCH",
      reason: "SOURCE_LESSONS_JSON_IS_NOT_VALID_JSON",
    };
  }
  const entries = sourceLessonEntries(parsed);
  const identityMatchCount = (entries ?? []).filter((entry) => {
    if (!entry || typeof entry !== "object" || Array.isArray(entry)) return false;
    return [entry.id, entry.lessonId, entry.sourceLessonId].some(
      (value) => value === input.sourceLessonId,
    );
  }).length;
  if (identityMatchCount !== 1) {
    return {
      ...
      actualLessonsJsonSha256,
      status: "IDENTITY_MISMATCH",
      reason: "SOURCE_LESSON_ID_NOT_UNIQUE_IN_SOURCE_DOCUMENT",
    };
  }
  const matches = actualLessonsJsonSha256 === input.expectedLessonsJsonSha256;
  return {
    sourceLessonId: input.sourceLessonId,
    expectedLessonsJsonSha256: input.expectedLessonsJsonSha256,
    actualLessonsJsonSha256,
    ...
    reason: matches
      ? "SOURCE_LESSONS_JSON_HASH_MATCHES_EXPECTATION"
      : "SOURCE_LESSONS_JSON_HASH_DIFFERS_FROM_EXPECTATION",
  };
}
...
function sourceLessonEntries(value) {
...
/** @param {RegistrySegment[]} [additionalSegments] @returns {MaterializationManifestEntry[]} */
export function buildIsmsPBatchAwareMaterializationManifest(
  additionalSegments = [],
) {
  const records = getIsmsPTheoryRegistryRecords(additionalSegments);
  if (additionalSegments.length === 0) assertExactBatch(records);
  return buildMaterializationManifest(records);
}

/** @param {RegistryRecord[]} records @returns {MaterializationManifestEntry[]} */
function buildMaterializationManifest(records) {
...
export async function planIsmsPBatch1Materialization(database, options = {}) {
  const manifest = options.segments
    ? buildIsmsPBatchAwareMaterializationManifest(options.segments)
  ...
  return {
    ...
    expectedOperationSlots: manifest.length * 3,
    historicalBatch: !options.segments,
    operations,
    counts,
...
export async function applyIsmsPBatch1Materialization(
  database,
  context,
  options = {},
) {
  assertNonProductionWriteContext(database, context);
  const plan = await planIsmsPBatch1Materialization(database, options);
  const { statements, createdOperationIds } =
    buildIsmsPBatch1CreateStatements(plan, options);
  ...
  const verification = await verifyIsmsPBatch1Materialization(database, options);
  return {
    mode: "APPLY",
    plan,
...
export function buildIsmsPBatch1CreateStatements(plan, options = {}) {
...
export async function verifyIsmsPBatch1Materialization(database, options = {}) {
  const plan = await planIsmsPBatch1Materialization(database, options);
  const verified =
    plan.course.valid &&
    plan.counts.CREATE === 0 &&
    plan.counts.NOOP === plan.expectedOperationSlots &&
...
function assertApplicablePlan(plan) {
  ...
  if (
    plan.operationSlots !== plan.expectedOperationSlots ||
    plan.holdOperationCount !== 0 ||
    (plan.historicalBatch && plan.operationSlots !== 36)
  ) {
    throw materializerError(
      "ISMS_P_BATCH1_OPERATION_SCOPE_INVALID",
      "The materialization plan must contain its derived approved slots and no HOLD operations.",
    );
  }
