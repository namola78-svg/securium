...
export const SECURITY_CONTENT_V3_CONTENT_VERSION = "3.0.0";
...
export function securityContentV3ContentProjection(row) {
  return {
    id: row.id,
    version: row.version ?? SECURITY_CONTENT_V3_CONTENT_VERSION,
    title: row.title ?? "",
    summary: row.summary ?? "",
    body: row.body ?? "",
    bodyFormat: row.bodyFormat ?? row.body_format ?? "STRUCTURED_JSON",
    learningObjectivesJson:
      row.learningObjectivesJson ?? JSON.stringify(row.learningObjectives ?? []),
    coreConceptsJson:
      row.coreConceptsJson ?? JSON.stringify(row.coreConcepts ?? []),
    practicalExamplesJson:
      row.practicalExamplesJson ?? JSON.stringify(row.practicalExamples ?? []),
    diagramsJson: row.diagramsJson ?? JSON.stringify(row.diagrams ?? []),
    mediaJson: row.mediaJson ?? JSON.stringify(row.media ?? []),
  };
}
...
export function generateSecurityContentV3Sql(
  input,
  { dialect, actorId = "user-content-editor", transactionBoundary = "generated" },
) {
  if (!new Set(["d1", "postgres"]).has(dialect)) {
    throw new Error(`SECURITY_CONTENT_V3_DIALECT_INVALID:${dialect}`);
  }
  if (!new Set(["generated", "caller"]).has(transactionBoundary)) {
    throw new Error(`SECURITY_CONTENT_V3_TRANSACTION_BOUNDARY_INVALID:${transactionBoundary}`);
  }
  if (transactionBoundary === "caller" && dialect !== "postgres") {
    throw new Error("SECURITY_CONTENT_V3_CALLER_TRANSACTION_POSTGRES_ONLY");
  }
  const plan = buildSecurityContentV3Plan(input);
  const statements = transactionBoundary === "generated"
    ? [dialect === "postgres" ? "BEGIN;" : "BEGIN TRANSACTION;"]
    : [];
  if (dialect === "postgres") statements.push(lockExistingContentRows(plan.contents));

  for (const content of plan.contents) statements.push(upsertContent(content, actorId));
  statements.push(immutableContentConflictGuard(plan.contents, dialect));
  ...
  if (transactionBoundary === "generated") statements.push("COMMIT;");
...
export function generateSecurityContentIntelligenceV3Sql({ dialect, actorId = "user-content-editor", transactionBoundary = "generated" }) {
  if (!new Set(["d1", "postgres"]).has(dialect)) throw new Error(`SECURITY_CONTENT_INTELLIGENCE_V3_DIALECT_INVALID:${dialect}`);
  if (!new Set(["generated", "caller"]).has(transactionBoundary)) {
    throw new Error(`SECURITY_CONTENT_INTELLIGENCE_V3_TRANSACTION_BOUNDARY_INVALID:${transactionBoundary}`);
  }
  if (transactionBoundary === "caller" && dialect !== "postgres") {
    throw new Error("SECURITY_CONTENT_INTELLIGENCE_V3_CALLER_TRANSACTION_POSTGRES_ONLY");
  }
...
function upsertContent(row, actorId) {
  const conflict = `ON CONFLICT ("id") DO NOTHING`;
  return `INSERT INTO "contents" ("id","slug","canonical_key","title","summary","body","body_format","learning_objectives_json","core_concepts_json","practical_examples_json","version","status","created_by") VALUES (${q(row.id)},${q(row.slug)},${q(row.canonicalKey)},${q(row.title)},${q(row.summary)},${q(row.body)},'STRUCTURED_JSON',${q(JSON.stringify(row.learningObjectives))},${q(JSON.stringify(row.coreConcepts))},${q(JSON.stringify(row.practicalExamples))},'${SECURITY_CONTENT_V3_CONTENT_VERSION}','DRAFT',${q(actorId)}) ${conflict};`;
}

function lockExistingContentRows(contents) {
  const ids = contents.map((content) => q(content.id)).join(",");
  if (!ids) return `-- SECURITY_CONTENT_V3_CONTENT_LOCK\nSELECT "id" FROM "contents" WHERE 1=0 FOR UPDATE;`;
  return `-- SECURITY_CONTENT_V3_CONTENT_LOCK\nSELECT "id" FROM "contents" WHERE "id" IN (${ids}) FOR UPDATE;`;
}

function immutableContentConflictGuard(contents, dialect) {
  const distinctOperator = dialect === "postgres" ? "IS DISTINCT FROM" : "IS NOT";
  const conflicts = contents.map((content) => {
    const projection = securityContentV3ContentProjection(content);
    const fields = [
      ["version", projection.version],
      ["title", projection.title],
      ["summary", projection.summary],
      ["body", projection.body],
      ["body_format", projection.bodyFormat],
      ["learning_objectives_json", projection.learningObjectivesJson],
      ["core_concepts_json", projection.coreConceptsJson],
      ["practical_examples_json", projection.practicalExamplesJson],
      ["diagrams_json", projection.diagramsJson],
      ["media_json", projection.mediaJson],
    ];
    return `(existing."id" = ${q(projection.id)} AND (${fields
      .map(([column, value]) => `existing."${column}" ${distinctOperator} ${q(value)}`)
      .join(" OR ")}))`;
  }).join(" OR ");

  if (!conflicts) return "-- SECURITY_CONTENT_V3_IMMUTABLE_CONTENT_GUARD\nSELECT 1 WHERE 0;";
  // The insert is deliberately conditional and deliberately has no conflict
  // handler: if DO NOTHING skipped an immutable mismatch, duplicating the
  // already-present id aborts the surrounding transaction before mappings.
  return `-- SECURITY_CONTENT_V3_IMMUTABLE_CONTENT_GUARD\nINSERT INTO "contents" ("id","slug","canonical_key","title","summary","body","body_format","learning_objectives_json","core_concepts_json","practical_examples_json","diagrams_json","media_json","version","status","created_by")
SELECT existing."id",existing."slug",existing."canonical_key",existing."title",existing."summary",existing."body",existing."body_format",existing."learning_objectives_json",existing."core_concepts_json",existing."practical_examples_json",existing."diagrams_json",existing."media_json",existing."version",existing."status",existing."created_by"
FROM "contents" AS existing
WHERE ${conflicts}
LIMIT 1;`;
}
...
function scopedQuestionRelationshipCleanup(questionIds) {
  if (!questionIds.length) return "-- SECURITY_CONTENT_V3_QUESTION_RELATIONSHIP_CLEANUP\nSELECT 1 WHERE FALSE;";
