...
export const PRACTICAL_SECURITY_CONTENT_IDS = {
  ...
  engineerLogCollectionMonitoring:
...
function buildPracticalChoices(definition) {
  if (definition.choices == null) {
    if (definition.type !== "SHORT_ANSWER") {
      throw new TypeError(
        `${definition.id} requires raw choices for automatic grading`,
      );
    }
    return [];
  }
  ...
  if (!Array.isArray(definition.choices)) {
    throw new TypeError(`${definition.id} choices must be an array`);
  }

  const seenContents = new Set();
  let correctCount = 0;
  const choices = definition.choices.map((rawChoice, index) => {
    if (!Array.isArray(rawChoice) || rawChoice.length !== 3) {
      throw new TypeError(
        `${definition.id} choice ${index + 1} must be [content, isCorrect, explanation]`,
      );
    }

    const [content, isCorrect, explanation] = rawChoice;
    if (typeof content !== "string" || content.trim() === "") {
      throw new TypeError(`${definition.id} choice ${index + 1} content is invalid`);
    }
    if (typeof isCorrect !== "boolean") {
      throw new TypeError(`${definition.id} choice ${index + 1} correctness is invalid`);
    }
    if (typeof explanation !== "string") {
      throw new TypeError(`${definition.id} choice ${index + 1} explanation is invalid`);
    }
    if (seenContents.has(content)) {
      throw new TypeError(`${definition.id} contains duplicate choices`);
    }
  ...
  assertPracticalCorrectChoiceCount(definition, choices.length, correctCount);
  return choices;
}

function assertPracticalCorrectChoiceCount(definition, choiceCount, correctCount) {
  if (definition.type === "SHORT_ANSWER") return;
  if (definition.type === "TRUE_FALSE" && choiceCount !== 2) {
    throw new TypeError(`${definition.id} TRUE_FALSE choices must contain two choices`);
  }
  if (
    (definition.type === "TRUE_FALSE" || definition.type === "SINGLE_CHOICE") &&
    correctCount !== 1
  ) {
    throw new TypeError(`${definition.id} must contain exactly one correct choice`);
  }
  if (definition.type === "MULTIPLE_CHOICE" && correctCount < 2) {
    throw new TypeError(`${definition.id} must contain at least two correct choices`);
  }
}

function assertCanonicalPracticalChoices(question) {
  if (!question || typeof question.id !== "string" || question.id.length === 0) {
    throw new TypeError("practical grading question identity is required");
  }
  if (!Array.isArray(question.choices)) {
    throw new TypeError(`${question.id} choices must be normalized objects`);
  }
  ...
  for (const [index, choice] of question.choices.entries()) {
    if (!choice || typeof choice !== "object" || Array.isArray(choice)) {
      throw new TypeError(`${question.id} choice ${index + 1} is not normalized`);
    }
    if (
      typeof choice.id !== "string" ||
      choice.id.length === 0 ||
      !choice.id.startsWith(`${question.id}-`)
    ) {
      throw new TypeError(`${question.id} choice ${index + 1} has invalid identity`);
    }
    if (seenIds.has(choice.id)) {
      throw new TypeError(`${question.id} contains duplicate choice identities`);
    }
    if (typeof choice.content !== "string" || choice.content.trim() === "") {
      throw new TypeError(`${question.id} choice ${index + 1} content is invalid`);
    }
    if (seenContents.has(choice.content)) {
      throw new TypeError(`${question.id} contains duplicate choices`);
    }
    if (choice.displayOrder !== index + 1) {
      throw new TypeError(`${question.id} choice order is not deterministic`);
    }
    if (typeof choice.isCorrect !== "boolean") {
      throw new TypeError(`${question.id} choice ${index + 1} correctness is invalid`);
    }
    if (typeof choice.explanation !== "string") {
      throw new TypeError(`${question.id} choice ${index + 1} explanation is invalid`);
    }

    seenIds.add(choice.id);
    seenContents.add(choice.content);
    if (choice.isCorrect) correctCount += 1;
  }

  assertPracticalCorrectChoiceCount(
    question,
...
const activeEngineerPracticalLogMonitoringQuestion = {
  ...engineerPracticalLogMonitoringQuestion,
  status: "PUBLISHED",
  authoringOnly: false,
  ...
  contentLinks: [
    {
      contentType: "CONTENT",
      contentId: PRACTICAL_SECURITY_CONTENT_IDS.engineerLogCollectionMonitoring,
      relationType: "PRACTICE",
    },
  ],
};
...
export const practicalSecurityQuestionSamples = [
  ...
  activeEngineerPracticalLogMonitoringQuestion,
];
...
export function toPracticalSecurityGradingQuestion(question) {
  const choices = assertCanonicalPracticalChoices(question);
  return {
    type: question.type,
