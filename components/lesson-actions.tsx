...
export function LessonActions({
  ...
  const savedPositionRef = useRef(initialLastPosition);
  ...
  async function run(
    ...
    try {
      const response = await fetch("/api/lessons/progress", {
        ...
        body: JSON.stringify({ lessonId, action, lastPosition: position }),
      });
      ...
      if (typeof payload.result?.lastPosition === "number") {
        ...
        savedPositionRef.current = payload.result.lastPosition;
      }
  ...
  useEffect(() => {
    if (initialStatus === "NOT_STARTED" && !startedRef.current) {
      ...
      void run("START", initialLastPosition, true);
    }
    // Opening a lesson starts it once; subsequent state updates must not replay it.
  ...
  useEffect(() => {
    function recordPosition() {
      ...
      const position =
        scrollable <= 0
          ...
          : Math.min(
              10000,
              Math.max(0, Math.round((window.scrollY / scrollable) * 10000)),
            );
      setLastPosition((current) => Math.max(current, position));
      if (
        position >= savedPositionRef.current + 1000 ||
        (position === 10000 && savedPositionRef.current < 10000)
      ) {
        savedPositionRef.current = position;
        void run(
          completionPolicy === "SCROLL_END" && position === 10000
            ? "COMPLETE"
            : "UPDATE",
          position,
          true,
    ...
    // The listener intentionally persists for this lesson only.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lessonId]);

  const scrollRequired =
    completionPolicy === "SCROLL_END" && lastPosition < 10000;
  ...
  return (
    <div className="lesson-actions">
      ...
      <span className="lesson-reading-progress">
        읽기 위치 {Math.round(lastPosition / 100)}%
      </span>
      <button
        className="button button-dark"
        type="button"
        disabled={pending || status === "COMPLETED" || scrollRequired}
        onClick={() => run("COMPLETE", lastPosition)}
      >
        {pending
          ? "저장 중…"
          : status === "COMPLETED"
