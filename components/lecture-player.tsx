...
export function LecturePlayer({ lectureId, title, durationSeconds, embed, authenticated, initialPosition, initialCompleted, initialBookmarked, initialNote }: { lectureId: string; title: string; durationSeconds: number; embed: EmbedConfig; authenticated: boolean; initialPosition: number; initialCompleted: boolean; initialBookmarked: boolean; initialNote: string }) {
  ...
  async function persistProgress(nextPosition: number, complete = false) {
    ...
    const response = await fetch("/api/lectures/progress", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ lectureId, currentPositionSeconds: Math.max(0, Math.min(durationSeconds, Math.round(nextPosition))), complete }) });
    const payload = (await response.json()) as { result?: { completed: boolean; currentPositionSeconds: number }; error?: string };
    if (!response.ok) { setMessage(payload.error ?? "시청 진도를 저장하지 못했습니다."); return; }
  ...
  function queueProgress(nextPosition: number, complete = false) {
    ...
    if (complete || elapsed >= 15_000) { if (saveTimerRef.current) clearTimeout(saveTimerRef.current); saveTimerRef.current = null; void persistProgress(nextPosition, complete); return; }
  ...
  useEffect(() => {
    function receiveMessage(event: MessageEvent) {
      ...
      let ended = false;
      if (embed.playerProtocol === "YOUTUBE_POST_MESSAGE" && message.event === "infoDelivery") { next = typeof message.info?.currentTime === "number" ? message.info.currentTime : null; ended = message.info?.playerState === 0; }
      if (embed.playerProtocol === "VIMEO_POST_MESSAGE") { if (message.event === "timeupdate" && typeof message.data?.seconds === "number") next = message.data.seconds; ended = message.event === "ended"; }
      if (next !== null) { const bounded = Math.max(0, Math.min(durationSeconds, Math.floor(next))); setPosition(bounded); queueProgress(bounded); }
      if (ended) { setPosition(durationSeconds); queueProgress(durationSeconds, true); }
    }
