...
export default async function PrivacyAssessmentPage({
  ...
  let data;
  try {
    data = await getPrivacyScenarioForUser(
      user.id,
      course.id,
      scenarioId,
    );
  ...
  return (
    <main className="page-main practical-page">
      <header className="page-hero practical-hero">
        <div className="shell">
          <p className="eyebrow">{data.scenario.track === "EXAM_PREP" ? "ASSESSOR EXAM PREP" : "PIA PRACTICE"}</p>
          <h1>{data.scenario.title}</h1>
          <p>{data.scenario.description}</p>
          <p className="sample-notice">독립 작성한 개발용 시나리오입니다. 운영 판단에는 최신 공식 기준 확인이 필요합니다.</p>
        </div>
      </header>
      <div className="shell privacy-scenario-layout">
        <section className="scenario-facts" aria-label="시나리오 기본 정보">
          <div><span>기관 유형</span><strong>{data.scenario.organizationType}</strong></div>
          <div><span>시스템 유형</span><strong>{data.scenario.systemType}</strong></div>
          <div><span>정보주체</span><strong>{data.scenario.dataSubjects}</strong></div>
          <div><span>처리 목적</span><strong>{data.scenario.processingPurpose}</strong></div>
          <div className="wide"><span>처리 개인정보</span><strong>{data.scenario.processedData}</strong></div>
        </section>
