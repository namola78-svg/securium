    ...
    <main className="page-main specialized-page">
      ...
      <div className="shell">
        ...
        {overview.standards.length ? (
          <ContentSection title="인증기준 탐색" description="기준 번호, 분류, 기준일과 버전을 DB에서 조회합니다.">
            {overview.standards.map((standard) => (
              <Link className="specialized-card" href={`/specialized/${courseSlug}/ISMS_STANDARD/${standard.id}`} key={standard.id}>
                <span className="badge">{standard.code}</span>
                <h3>{standard.title}</h3>
                <p>{standard.majorCategory} · {standard.middleCategory}</p>
                <small>버전 {standard.version} · 기준일 {standard.effectiveDate}</small>
              </Link>
            ))}
          </ContentSection>
        ) : null}
        ...
        {overview.defectCases.length ? (
          <ContentSection title="결함사례 학습" description="가상 상황에서 결함, 증적과 시정조치를 판단합니다.">
            {overview.defectCases.map((item) => (
              <Link className="specialized-card" href={`/specialized/${courseSlug}/ISMS_DEFECT_CASE/${item.id}`} key={item.id}>
        ...
        {overview.legalArticles.length ? (
          <ContentSection title="법령·조문 비교 학습" description="같은 법령의 조문과 버전, 시행·개정 기준일을 비교합니다.">
            {overview.legalArticles.map((article) => (
              <Link className="specialized-card" href={`/specialized/${courseSlug}/LEGAL_ARTICLE/${article.id}`} key={article.id}>
                <span className="badge">{article.articleNumber}</span>
                <h3>{article.articleTitle}</h3>
                <p>{article.lawName}</p>
                <small>시행 {article.effectiveDate} · 개정 {article.revisionDate} · {article.version}</small>
              </Link>
            ))}
          </ContentSection>
        ) : null}
        ...
        {overview.writtenQuestions.length ? (
          <section className="section-block">
            <div className="section-heading"><div><p className="eyebrow">WRITTEN PRACTICE</p><h2>서술형 참고용 보조채점</h2></div></div>
            <p className="sample-notice">공식 채점이 아니며 필수·선택 키워드 충족 여부를 학습 참고용으로만 제공합니다.</p>
        ...
        {overview.riskScenarios.length ? (
          <>
            <ContentSection title="위험 시나리오" description="자산·위협·취약점과 기존 통제를 연결해 평가합니다.">
              {overview.riskScenarios.map((scenario) => (
                <Link className="specialized-card" href={`/specialized/${courseSlug}/RISK_SCENARIO/${scenario.id}`} key={scenario.id}>
                  <span className="badge">{scenario.riskLevel}</span>
                  <h3>{scenario.title}</h3>
                  <p>{scenario.asset} · {scenario.threat}</p>
                  <small>위험값 {scenario.riskValue} · 기준일 {scenario.referenceDate}</small>
                </Link>
              ))}
            </ContentSection>
            ...
            <section className="admin-panel section-block">
              ...
              {riskRegister.length ? riskRegister.map((item) => (
                <div className="risk-register-row" key={item.id}>
                  <div><strong>{item.scenarioTitle}</strong><small>{item.asset} · {item.owner}</small></div>
                  <span>{item.riskValue}</span><span className="badge">{item.status}</span>
                </div>
...
function ContentSection({
  ...
  return (
    <section className="section-block">
      <div className="section-heading compact"><div><p className="eyebrow">SPECIALIZED CONTENT</p><h2>{title}</h2><p>{description}</p></div></div>
