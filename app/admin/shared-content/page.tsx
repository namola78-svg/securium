  ...
  return (
    <>
      <header className="admin-page-header">
        ...
        <h1>공통 콘텐츠 공유 관리</h1>
        <p>
          하나의 학습 원문을 여러 과정에 재사용하고, 과정별 표시 제목·시험
          포인트·실무 메모는 CourseLesson과 Extension으로 분리합니다. 기존
          Lesson과 사용자 진도는 삭제하거나 변경하지 않습니다.
        </p>
      </header>
      <section className="stats-grid admin-stats">
        <div className="stat-card">
          ...
          <small>PUBLISHED {contents.filter((item) => item.status === "PUBLISHED").length}</small>
        </div>
        <div className="stat-card">
          <span>선택 과정 레슨</span>
          <strong>{courseLessons.length}</strong>
          <small>{courses.find((course) => course.id === selectedCourseId)?.shortName ?? "없음"}</small>
        </div>
        <div className="stat-card">
          <span>커리큘럼 노드</span>
          <strong>{curriculumNodes.length}</strong>
          <small>{activeTree ? `${activeTree.title} · ${activeTree.status}` : "트리 없음"}</small>
        </div>
      </section>
