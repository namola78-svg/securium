...
export default async function LectureDetailPage({ params }: { params: Promise<{ courseSlug: string; lectureId: string }> }) {
  ...
  return <main className="page-main"><section className="page-hero lecture-detail-hero"><div className="shell narrow"><Link className="breadcrumb" href={`/lectures/${course.slug}`}>← 강의 목록</Link><div className="course-card-top"><span className="badge">{lecture.free ? "무료 강의" : "수강 전용"}</span>{lecture.isSample ? <span className="sample-label">강의 영상</span> : null}</div><h1>{publicCopy(lecture.title)}</h1><p>{publicCopy(lecture.description)}</p><div className="lesson-meta"><span>{publicCopy(lecture.subjectName)}</span><span>{publicCopy(lecture.topicName)}</span><span>{publicCopy(lecture.instructorName) || "강사 정보 없음"}</span><span>{Math.ceil(lecture.durationSeconds / 60)}분</span><span>{publicCopy(lecture.embed.providerLabel)}</span></div></div></section><section className="section"><div className="shell narrow"><LecturePlayer key={`${user?.id ?? "anonymous"}:${lecture.id}:${revision?.id ?? "none"}`} lectureId={lecture.id} title={publicCopy(lecture.title)} durationSeconds={lecture.durationSeconds} embed={lecture.embed} authenticated={Boolean(user)} initialPosition={lecture.currentPositionSeconds} initialCompleted={lecture.completed} initialBookmarked={lecture.bookmarked} initialNote={lecture.note ?? ""} contentRevisionId={revision?.id ?? null} persistedContentRevisionId={lecture.progressContentRevisionId ?? null} /><ContentVersionInfo revision={revision} /><div className="lecture-related-grid section-block"><RelatedSection title="관련 이론" items={lecture.relatedTheory.map((lesson) => ({ id: lesson.id, title: publicCopy(lesson.title), description: publicCopy(lesson.summary), href: `/learn/${course.slug}/lessons/${lesson.id}` }))} empty="연결된 이론 자료가 없습니다." /><RelatedSection title="관련 문제" items={lecture.relatedQuestions.map((question) => ({ id: question.id, title: publicCopy(question.title), description: `${question.type} · ${question.difficulty}`, href: `/practice/${course.slug}?questionId=${question.id}` }))} empty="연결된 문제가 없습니다." /></div><nav className="lesson-navigation" aria-label="강의 이동"><Link className="button button-ghost" href={`/lectures/${course.slug}`}>강의 목록</Link>{lecture.nextLecture ? <Link className="button button-dark" href={`/lectures/${course.slug}/${lecture.nextLecture.id}`}>다음 강의 · {publicCopy(lecture.nextLecture.title)}</Link> : <Link className="button button-dark" href={`/learn/${course.slug}`}>과정 학습으로</Link>}</nav></div></section></main>;
}
