...
- source contract가 없거나 일부만 있는 경우: `출처 정보 확인 필요`
...
4. 값이 없는 항목: `확인되지 않음`, `출처 정보 확인 필요`, `검토 정보 확인 필요` 중 실제 상태에 맞는 문구

`준비 중`은 실제 준비 작업이나 계획이 확인된 상태에서만 사용한다. 정보 부재만 확인된 경우에는 `출처 정보 확인 필요`처럼 확인되지 않은 상태를 그대로 표시한다.
...
아래 문구는 후속 구현을 위한 제안이며, 현재 DB의 실제 값을 채운 결과가 아니다. 대괄호 값은 승인된 데이터가 확보된 뒤에만 치환한다.
...
다음 조건은 후속 구현을 위한 제안이며, 모두 충족될 때만 공개 표시 구현을 완료로 본다.
...
- 값이 없는 상태가 `UNKNOWN` 또는 실제 준비 작업·계획이 확인된 경우에만 `준비 중`으로 안전하게 표시되고 낙관적 문구로 대체되지 않음
