...
| 과목 / SUBJECT | 9 | 9 | 9 |
| 실기 / PRACTICAL | 2 | 2 | 2 |
| 주요항목 / MAJOR_ITEM | 33 | 0 | 0 |
| 세부항목 / SUB_ITEM | 95 | 0 | 0 |

이 수치는 운영 DB 상태가 아니라 로컬 seed 코드 기준이다. 다음 콘텐츠 확장은 우선순위가 높은 `MAJOR_ITEM`부터 분할하고, 이후 `SUB_ITEM` 단위로 본문·문항을 세분화한다.
...
1. `MAJOR_ITEM` 단위의 세부 본문 분할
2. `SUB_ITEM` 단위의 세부 본문과 문항 확대
