...
최상위 노드 기준 커버리지는 100%이며, 네트워크 보안, 시스템 보안, 애플리케이션 보안 일부는 공식 출제기준의 `MAJOR_ITEM` / `SUB_ITEM` 단위 Content로 확장되어 있다. 현재 로컬 계측 기준은 다음과 같다.

| 범위 | 학습 노드 | Content 연결 | 문항 연결 | 커버리지 |
| --- | ---: | ---: | ---: | ---: |
| 정보보안기사 | 77 | 72 | 76 | Content 93.5% / 문항 98.7% |
| 정보보안산업기사 | 62 | 59 | 61 | Content 95.2% / 문항 98.4% |
| 합계 | 139 | 131 | 137 | Content 94.2% / 문항 98.6% |
...
| 과목 / SUBJECT | 9 | 9 | 9 |
| 실기 / PRACTICAL | 2 | 2 | 2 |
| 주요항목 / MAJOR_ITEM | 33 | 25 | 33 |
| 세부항목 / SUB_ITEM | 95 | 95 | 95 |

이 수치는 운영 DB 상태가 아니라 로컬 seed 코드 기준이다. 네트워크 보안은 주요항목 3개와 세부항목 9개가 기사·산업기사 양쪽 CourseLesson으로 분할되었고, 9개 세부항목 모두 최소 1개 이상의 독립 작성 샘플 문항에 연결되어 있다. 시스템 보안은 주요항목 3개와 세부항목 8개가 공유 Content로 분할되었고, 8개 세부항목 모두 최소 1개 이상의 독립 작성 샘플 문항에 연결되어 있다. 애플리케이션 보안은 주요항목 2개와 세부항목 7개가 공유 Content로 분할되었고, 7개 세부항목 모두 최소 1개 이상의 독립 작성 샘플 문항에 연결되어 있다. 정보보안일반은 세부항목 7개가 공유 Content로 분할되었고, 7개 세부항목 모두 최소 1개 이상의 독립 작성 샘플 문항에 연결되어 있다. 정보보호관리 및 법규는 기사 전용 세부항목 7개가 Content로 분할되었고, 7개 세부항목 모두 최소 1개 이상의 독립 작성 샘플 문항에 연결되어 있다. 실기 주요항목·세부항목도 공유 Content와 기사 전용 위험관리 Content로 분할되어 CourseLesson에 연결되었고, 실기 세부항목 15개에는 최소 1개 이상의 독립 작성 샘플 문항이 연결되어 있다. 주요항목의 문항 커버리지는 중복 문항을 만들지 않고 하위 세부항목 문항을 집계하는 정책을 적용한다. 산업기사 CourseLesson으로는 기사 전용 관리·법규와 위험관리 실기 항목이 노출되지 않는다. CourseLesson과 진도는 기사·산업기사별로 분리된다. 남은 Content gap은 필기 주요항목 8개이며, 남은 문항 gap은 네트워크 보안 과목 개요 2개다.
...
1. 보안요소 기술·암호학·최신 보안 동향 주요항목 Content 분할
