...
| --- | ---: | ---: | ---: | ---: |
| 정보보안기사 | 77 | 9 | 6 | Content 11.7% / 문항 7.8% |
| 정보보안산업기사 | 62 | 8 | 5 | Content 12.9% / 문항 8.1% |
| 합계 | 139 | 17 | 11 | Content 12.2% / 문항 7.9% |
...
| 세부항목 / SUB_ITEM | 95 | 0 | 0 |

이 수치는 운영 DB 상태가 아니라 로컬 seed 코드 기준이다. 네트워크 보안 주요항목 3개는 기사·산업기사 양쪽 CourseLesson으로 분할되었고, 문항은 아직 최상위 네트워크 보안 Content에 연결되어 있다. 다음 콘텐츠 확장은 다른 과목의 `MAJOR_ITEM` 분할과 네트워크 보안 `SUB_ITEM` 단위 문항 연결이다.
...
1. 시스템 보안 `MAJOR_ITEM` 단위의 세부 본문 분할
2. 네트워크 보안 `SUB_ITEM` 단위의 세부 본문과 문항 확대
