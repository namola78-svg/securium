...
export async function listUserEnrollments(userId: string) {
  const enrollmentRows = await getDb()
    .select({
      ...
      theoryCompletedLessons: sql<number>`coalesce((select count(${userCourseLessonProgress.id}) from ${userCourseLessonProgress} inner join ${courseLessons} on ${userCourseLessonProgress.courseLessonId} = ${courseLessons.id} inner join ${contents} on ${courseLessons.contentId} = ${contents.id} where ${userCourseLessonProgress.userId} = ${userId} and ${userCourseLessonProgress.courseId} = ${courses.id} and ${userCourseLessonProgress.status} = 'COMPLETED' and ${userCourseLessonProgress.contentVersion} = ${contents.version} and ${courseLessons.courseId} = ${courses.id} and ${courseLessons.status} = 'PUBLISHED' and ${courseLessons.deletedAt} is null and ${contents.status} = 'PUBLISHED' and ${contents.deletedAt} is null), 0)`,
    })
    .from(userCourseEnrollments)
