...
export const evidenceProjections = sqliteTable(
  ...
  (table) => [
    ...
    index("evidence_projections_generation_idx").on(table.generationId, table.lifecycle),
...
export const evidenceRebuildGenerations = sqliteTable(
  "evidence_rebuild_generations",
  {
    id: text("id").primaryKey(),
    scopeKey: text("scope_key").notNull().default("EVIDENCE_V1"),
    projectionVersion: text("projection_version").notNull(),
    mappingSnapshotHash: text("mapping_snapshot_hash").notNull(),
    sourceCutoff: text("source_cutoff"),
  ...
  (table) => [
    uniqueIndex("evidence_rebuild_generations_active_unique")
      .on(table.scopeKey)
      .where(sql`${table.active} = 1`),
    index("evidence_rebuild_generations_status_idx").on(table.status, table.createdAt),
    check("evidence_rebuild_generations_status_check", sql`${table.status} IN ('PENDING', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELLED', 'SUPERSEDED')`),
    check("evidence_rebuild_generations_active_check", sql`${table.active} IN (0, 1)`),
  ],
);
...
export const evidenceRecomputeRequests = sqliteTable(
  ...
  (table) => [
    ...
    check("evidence_recompute_requests_status_check", sql`${table.status} IN ('PENDING', 'PROCESSING', 'RETRYABLE', 'COMPLETED', 'FAILED', 'CANCELLED', 'SUPERSEDED')`),
