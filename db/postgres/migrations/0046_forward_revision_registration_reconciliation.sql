-- Forward reconciliation: current generic content revision registration authority.
-- Reuses content_revisions; no second content revision authority or row backfill.
BEGIN;
...
CREATE FUNCTION public.content_revision_registration_immutable()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
BEFORE UPDATE ON public."content_revision_registrations"
FOR EACH ROW EXECUTE FUNCTION public.content_revision_registration_immutable();
CREATE TRIGGER content_revision_registrations_no_delete
BEFORE DELETE ON public."content_revision_registrations"
FOR EACH ROW EXECUTE FUNCTION public.content_revision_registration_immutable();
CREATE TRIGGER content_revision_registration_subjects_no_update
BEFORE UPDATE ON public."content_revision_registration_subjects"
FOR EACH ROW EXECUTE FUNCTION public.content_revision_registration_immutable();
CREATE TRIGGER content_revision_registration_subjects_no_delete
BEFORE DELETE ON public."content_revision_registration_subjects"
FOR EACH ROW EXECUTE FUNCTION public.content_revision_registration_immutable();
CREATE TRIGGER content_revision_registration_sources_no_update
BEFORE UPDATE ON public."content_revision_registration_sources"
FOR EACH ROW EXECUTE FUNCTION public.content_revision_registration_immutable();
CREATE TRIGGER content_revision_registration_sources_no_delete
BEFORE DELETE ON public."content_revision_registration_sources"
FOR EACH ROW EXECUTE FUNCTION public.content_revision_registration_immutable();
...
VALUES ('0046_forward_revision_registration_reconciliation', 'forward-revision-registration-reconciliation-v1')
