-- Additive generic authenticated review judgment infrastructure; no backfill or review execution.
BEGIN;
...
CREATE UNIQUE INDEX "content_review_judgments_semantic_unique" ON public."content_review_judgments" ("semantic_review_identity");
CREATE UNIQUE INDEX "content_review_judgments_idempotency_unique" ON public."content_review_judgments" ("idempotency_key");
CREATE INDEX "content_review_judgments_input_domain_idx" ON public."content_review_judgments" ("reviewed_input_identity", "review_domain", "lifecycle_state");
CREATE INDEX "content_review_judgments_supersession_idx" ON public."content_review_judgments" ("supersedes_judgment_id");
CREATE UNIQUE INDEX "content_review_judgments_single_successor_unique" ON public."content_review_judgments" ("supersedes_judgment_id") WHERE "supersedes_judgment_id" IS NOT NULL;
CREATE INDEX "content_review_judgments_conflict_slot_idx" ON public."content_review_judgments" ("conflict_slot_identity", "lifecycle_state");
CREATE OR REPLACE FUNCTION public.content_review_judgment_active_slot_guard()
...
BEGIN
  ...
  SELECT * INTO prior
   ...
   LIMIT 1;
  IF prior.judgment_id IS NOT NULL AND prior.semantic_review_identity <> NEW.semantic_review_identity THEN
...
CREATE INDEX "content_review_judgment_subjects_lookup_idx" ON public."content_review_judgment_subjects" ("subject_identity", "semantic_ordinal");
...
CREATE OR REPLACE FUNCTION public.content_review_judgment_history_immutable()
RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
  RAISE EXCEPTION 'Content review judgment history is append-only';
END;
$$;
CREATE TRIGGER content_review_judgments_no_update BEFORE UPDATE ON public."content_review_judgments" FOR EACH ROW EXECUTE FUNCTION public.content_review_judgment_history_immutable();
CREATE TRIGGER content_review_judgments_no_delete BEFORE DELETE ON public."content_review_judgments" FOR EACH ROW EXECUTE FUNCTION public.content_review_judgment_history_immutable();
CREATE TRIGGER content_review_judgment_subjects_no_update BEFORE UPDATE ON public."content_review_judgment_subjects" FOR EACH ROW EXECUTE FUNCTION public.content_review_judgment_history_immutable();
CREATE TRIGGER content_review_judgment_subjects_no_delete BEFORE DELETE ON public."content_review_judgment_subjects" FOR EACH ROW EXECUTE FUNCTION public.content_review_judgment_history_immutable();
CREATE TRIGGER content_review_findings_no_update BEFORE UPDATE ON public."content_review_findings" FOR EACH ROW EXECUTE FUNCTION public.content_review_judgment_history_immutable();
CREATE TRIGGER content_review_findings_no_delete BEFORE DELETE ON public."content_review_findings" FOR EACH ROW EXECUTE FUNCTION public.content_review_judgment_history_immutable();
REVOKE ALL PRIVILEGES ON TABLE public."content_review_judgments", public."content_review_judgment_subjects", public."content_review_findings" FROM PUBLIC, anon, authenticated;
...
INSERT INTO app_schema_migrations (id, checksum) VALUES ('0024_content_review_judgment_infrastructure', 'content-review-judgment-infrastructure-v1') ON CONFLICT (id) DO NOTHING;
COMMIT;
