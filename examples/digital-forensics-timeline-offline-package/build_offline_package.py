"""Build and verify a deterministic offline copy of the timeline lab.
...
PACKAGE_FORMAT = "securium-forensics-timeline-offline-package-v1"
INTERNAL_MANIFEST_NAME = "package-manifest.json"
DEFAULT_ZIP_NAME = "securium-forensics-timeline-offline-package.zip"
DEFAULT_EXTERNAL_MANIFEST_NAME = "securium-forensics-timeline-offline-package.manifest.json"
...
# This is the complete source allowlist.  The builder and its package-boundary
# tests stay in the repository and are intentionally not extracted for learners.
PACKAGE_ALLOWLIST: tuple[tuple[str, str], ...] = (
    (
        "examples/digital-forensics-timeline-local-lab/README.md",
        "examples/digital-forensics-timeline-local-lab/README.md",
    ),
    (
    ...
    (
        "examples/digital-forensics-timeline-local-lab/instructor-guide.md",
        "examples/digital-forensics-timeline-local-lab/instructor-guide.md",
    ),
    (
        "examples/digital-forensics-timeline-local-lab/learner-guide.md",
        "examples/digital-forensics-timeline-local-lab/learner-guide.md",
    ),
    (
        "examples/digital-forensics-timeline-local-lab/run_tests.py",
        "examples/digital-forensics-timeline-local-lab/run_tests.py",
    ),
    (
        "examples/digital-forensics-timeline-local-lab/test_timeline_lab.py",
        "examples/digital-forensics-timeline-local-lab/test_timeline_lab.py",
    ),
    (
    ...
    (
        "examples/digital-forensics-timeline-local-lab/rehearsal-kit/assessment-rubric.md",
        "examples/digital-forensics-timeline-local-lab/rehearsal-kit/assessment-rubric.md",
    ),
    (
        "examples/digital-forensics-timeline-local-lab/rehearsal-kit/instructor-runbook.md",
        "examples/digital-forensics-timeline-local-lab/rehearsal-kit/instructor-runbook.md",
    ),
    (
        "examples/digital-forensics-timeline-local-lab/rehearsal-kit/learner-workbook.md",
        "examples/digital-forensics-timeline-local-lab/rehearsal-kit/learner-workbook.md",
    ),
    (
    ...
    for entry in entries:
        ...
        for match in _MARKDOWN_LINK.finditer(text):
            ...
            if target is None:
                ...
                continue
            package_directory = target.rstrip("/")
            if target in package_paths or any(
                path.startswith(f"{package_directory}/") for path in package_paths
            ):
                internal.append({"document": document, "target": target, "kind": "package-relative"})
            elif (
                target == "content-drafts"
                or target.startswith("content-drafts/")
                or target == "examples/digital-forensics-integrity-local-lab"
                or target.startswith("examples/digital-forensics-integrity-local-lab/")
            ):
                external.append(
                    {
                        "document": document,
    ...
    *,
    ...
    try:
        ...
        if trusted_source_commit is None:
            raise PackageError("trusted source commit is required for source-bound verification")
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build or verify the forensic timeline offline package")
