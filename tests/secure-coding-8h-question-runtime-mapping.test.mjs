...
test("builds exactly 40 deterministic Foundation-bound mappings", async () => {
  const first = await buildSecureCoding8HQuestionRuntimeMapping();
  const second = await buildSecureCoding8HQuestionRuntimeMapping();
...
test("uses deterministic runtime and version identities and preserves answer order", async () => {
  ...
  const mapping = (await buildSecureCoding8HQuestionRuntimeMapping()).mappings[0];
...
test("all 40 projections remain compatible with the shared single-choice grader", async () => {
  const result = await buildSecureCoding8HQuestionRuntimeMapping();
...
test("revalidates the generated projection and detects runtime identity drift", async () => {
  ...
  const result = await projectSecureCoding8HQuestionRuntimeMapping(sourceModel);
  await assertSecureCoding8HQuestionRuntimeMapping(result, sourceModel);
  const wrongRuntimeId = clone(result);
  wrongRuntimeId.mappings[0].runtimeQuestionId = "wrong-runtime-id";
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(wrongRuntimeId, sourceModel),
...
test("fails closed for cardinality, duplicate, Q41, and unknown Foundation IDs", async () => {
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(removed),
    "COUNT_MISMATCH",
  );
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(duplicate),
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(q41),
    "QUESTION_ID_CONFLICT",
  );
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(unknown),
    "QUESTION_ID_CONFLICT",
  );
});
...
test("fails closed for identity, version, type, answer, content, and binding mutations", async () => {
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(wrongCourse),
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(wrongCandidate),
    "RUNTIME_IDENTITY_MISMATCH",
  );
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(unsupportedType),
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(answerMutation),
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(explanationMutation),
    "FOUNDATION_INVALID",
  );
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(moduleMutation),
    "FOUNDATION_INVALID",
  );
  ...
  await expectCode(
    () => projectSecureCoding8HQuestionRuntimeMapping(objectiveMutation),
    "FOUNDATION_INVALID",
  );
});
...
test("fails closed for candidate version, semantic hash, answer, choice order, and missing mappings", async () => {
  const sourceModel = model();
  const base = await projectSecureCoding8HQuestionRuntimeMapping(sourceModel);

  const wrongVersion = clone(base);
  wrongVersion.mappings[0].runtimeQuestionVersionId = "version-wrong-v1";
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(wrongVersion, sourceModel),
    "MAPPING_PROJECTION_MISMATCH",
  );
  ...
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(wrongFoundationVersion, sourceModel),
    "MAPPING_PROJECTION_MISMATCH",
  );
  ...
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(hashMutation, sourceModel),
    "MAPPING_PROJECTION_MISMATCH",
  );
  ...
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(answerMutation, sourceModel),
    "MAPPING_PROJECTION_MISMATCH",
  );
  ...
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(orderMutation, sourceModel),
    "MAPPING_PROJECTION_MISMATCH",
  );
  ...
  await expectCode(
    () => assertSecureCoding8HQuestionRuntimeMapping(missing, sourceModel),
    "MAPPING_PROJECTION_MISMATCH",
  );
});
...
test("semantic hash changes for assessment content and ignores no runtime database state", async () => {
  const sourceModel = model();
  const baseline = await projectSecureCoding8HQuestionRuntimeMapping(sourceModel);

  const promptMutation = clone(sourceModel);
  promptMutation.foundation.questions.questions[0].prompt += " Revised";
  const changed = await projectSecureCoding8HQuestionRuntimeMapping(promptMutation);
  assert.notEqual(
