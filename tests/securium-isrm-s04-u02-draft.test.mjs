...
import { validateDraft } from "../scripts/validate-securium-isrm-s04-u02-draft.mjs";

test("S04-U02 draft validates as review-only content", async () => {
  const result = await validateDraft();
  assert.deepEqual(result, {
    status: "DRAFT_UNPUBLISHED_REVIEW_REQUIRED",
    learningUnitId: "isrm-2025-2027-s04-u02",
    objectiveCount: 2,
    theoryCount: 1,
    questionCount: 2,
    claimCount: 5,
...
test("S04-U02 draft keeps canonical and runtime boundaries closed", async () => {
  const readme = await readFile("content-drafts/securium-isrm-s04-u02-authoring/README.md", "utf8");
  const manifest = JSON.parse(await readFile("content-drafts/securium-isrm-s04-u02-authoring/manifest.json", "utf8"));
