...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s02-u02-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s02_u02.authoring_draft.v1");
  ...
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s02-u02");
  ...
  assert.deepEqual(manifest.officialScopeAnchor.subitems, [
    "정보자산 식별 및 중요도 평가",
    "위협 분류, 식별 및 발생 가능성 등 평가",
    "취약점 식별 및 평가",
    "법적 준거성 식별 및 평가",
    "종합 위험평가 및 목표 위험수준 평가",
  ]);
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-09", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-10", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-11", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S02-U02-O01", "ISRM-S02-U02-O02"]);
  unique(objectiveIds, "objective IDs");
  assert.deepEqual(objectives.objectives[0].questionIds, ["isrm-q-v2-09"]);
  assert.deepEqual(objectives.objectives[1].questionIds, ["isrm-q-v2-10", "isrm-q-v2-11"]);
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S02-U02-T01"]);
  }

  assert.equal(theory.schema, "securium.isrm.s02_u02.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S02-U02-T01");
  ...
  const bridge = theory.sections.find((section) => section.id === "bridge-from-u01");
  assert.ok(bridge.body.includes("S02-U01"), "theory must connect to S02-U01");

  const questionsById = new Map(questions.questions.map((question) => [question.id, question]));
  const expectedQuestionIds = ["isrm-q-v2-09", "isrm-q-v2-10", "isrm-q-v2-11"];
