...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s04-u01-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s04_u01.authoring_draft.v1");
  ...
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s04-u01");
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-21", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-22", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S04-U01-O01", "ISRM-S04-U01-O02"]);
  unique(objectiveIds, "objective IDs");
  assert.deepEqual(objectives.objectives[0].questionIds, ["isrm-q-v2-21"]);
  assert.deepEqual(objectives.objectives[1].questionIds, ["isrm-q-v2-22"]);
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S04-U01-T01"]);
  }

  assert.equal(theory.schema, "securium.isrm.s04_u01.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S04-U01-T01");
  ...
  const bridge = theory.sections.find((section) => section.id === "bridge-from-s03");
  for (const prior of ["S03-U01", "S03-U02", "S03-U03", "S03-U04"]) {
  ...
  const expectedQuestionIds = ["isrm-q-v2-21", "isrm-q-v2-22"];
