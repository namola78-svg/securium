...
if (!VALID_TARGETS.has(target)) {
  fail("SYSTEM_SECURITY_QUESTION_FLOW_TARGET_INVALID");
}
...
async function verifyD1Local() {
  ...
  console.log("SYSTEM_SECURITY_QUESTION_FLOW_D1_LOCAL_OK");
}
...
async function verifyPostgres() {
  ...
  const sql = postgres(connectionUrl, {
    ...
    debug: false,
    connection: {
      application_name: "securium-system-security-question-flow-verify",
    },
  });
  ...
  try {
    ...
    fail("SYSTEM_SECURITY_QUESTION_FLOW_POSTGRES_FAILED", safeErrorCode(error));
  } finally {
    await sql.end({ timeout: 5 });
  }

  console.log("SYSTEM_SECURITY_QUESTION_FLOW_POSTGRES_OK");
}
...
function buildVerificationSql(dialect) {
  ...
  const contentId = sqlString(SYSTEM_SECURITY_CONTENT_ID);
...
course_counts AS (
  ...
  INNER JOIN system_questions q ON q.id = qc.question_id
...
content_links AS (
  ...
  INNER JOIN system_questions q ON q.id = cql.question_id
...
function verifyRows(rows) {
  ...
  assertCount(
    rowMap,
    `content:${SYSTEM_SECURITY_CONTENT_ID}`,
  ...
  for (const courseId of SYSTEM_SECURITY_COURSE_IDS) {
    ...
    JSON.stringify(
      {
        ...
        courseIds: SYSTEM_SECURITY_COURSE_IDS,
        rows: normalizedRows,
      },
      null,
      2,
...
async function d1Query(configPath, statement) {
  ...
  if (result.code !== 0) {
    fail("SYSTEM_SECURITY_QUESTION_FLOW_D1_FAILED");
  }
...
function parseWranglerResults(stdout) {
  ...
  if (start < 0 || end < start) {
    fail("SYSTEM_SECURITY_QUESTION_FLOW_D1_JSON_MISSING");
  }
  ...
  try {
    ...
    fail("SYSTEM_SECURITY_QUESTION_FLOW_D1_JSON_INVALID");
  }
}
...
function assertCount(rowMap, key, expected) {
  ...
  if (actual !== expected) {
    fail(
      "SYSTEM_SECURITY_QUESTION_FLOW_COUNT_MISMATCH",
