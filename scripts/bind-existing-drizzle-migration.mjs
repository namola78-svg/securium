...
export async function bindExistingMigration({ existingSql, expectedSha256, apply = false, rootDir = root, failureStage }) {
  ...
  await ensureFile(sqlPath, "existing SQL"); const sql = await readFile(sqlPath, "utf8"); const migrationPath = relative(repoRoot, sqlPath).replaceAll("\\", "/"); let canonicalBlob; try { canonicalBlob = canonicalMigrationBlob({ revision: "HEAD", path: migrationPath, repoRoot }); } catch (error) { throw new BindingError(error.code ?? "CANONICAL_MIGRATION_HASH_FAILED", error.message, error.details ?? {}); } const actualSha = canonicalMigrationSha256({ revision: canonicalBlob.revision, path: canonicalBlob.path, repoRoot }); if (actualSha !== expectedSha256) throw new BindingError("MIGRATION_HASH_MISMATCH", `${actualSha} != ${expectedSha256}`); if (DRIZZLE_KIT_VERSION !== "0.31.10") throw new BindingError("UNSUPPORTED_DRIZZLE_VERSION", DRIZZLE_KIT_VERSION);
  ...
  const schemaSha = canonicalMigrationSha256({ revision: "HEAD", path: "db/schema.ts", repoRoot });
  const existingEntry = entries.find((entry) => entry.tag === identity.tag); if (existingEntry) { const snapshotPath = join(metaDir, `${identity.prefix}_snapshot.json`); await ensureFile(snapshotPath, "bound snapshot"); const snapshot = await readJson(snapshotPath); const priorSnapshot = await readJson(join(metaDir, `${String(identity.idx - 1).padStart(4, "0")}_snapshot.json`)); if (existingEntry.idx !== identity.idx || snapshot.prevId !== priorSnapshot.id) throw new BindingError("EXACT_REPLAY_METADATA_MISMATCH", identity.tag); if (!(await verifyBindingReceipt({ metaDir, binderDir, receiptFilename: BINDER_RECEIPT_FILENAME, identity: identityForRecovery, existingSqlSha256: actualSha, schemaSha256: schemaSha, repoRoot }))) throw new BindingError("EXACT_REPLAY_RECEIPT_MISSING", identity.tag); machine.transition("EXACT_REPLAY"); return { status: "EXACT_REPLAY", identity, existingSha256: actualSha, snapshot: { id: snapshot.id, prevId: snapshot.prevId }, journal: { idx: existingEntry.idx, tag: existingEntry.tag }, applied: false, binderStateHistory: machine.history }; }
  ...
  try {
    ...
    const diagnostic = classifySqlOperations(sql, generatedSql, targetSnapshot); let authority; try { authority = await runAuthorityComparison({ repoRoot, migrationIndex: identity.idx, authorizedSql: sql, generatedSql }); } catch (error) { machine.fail(); throw error; } machine.transition("EXECUTION_VALIDATED"); machine.transition("SCHEMA_EQUIVALENT"); machine.transition("DATA_EQUIVALENT"); const boundJournal = { ...journal, entries: [...entries, { ...generatedEntry, tag: identity.tag }] }; const snapshotText = stableJson(targetSnapshot); const journalText = stableJson(boundJournal);
    const receipt = { version: "PRACTICAL_BINDING_RECEIPT_V2", authorityRevision: canonicalBlob.revision, hashBasisVersion: CANONICAL_HASH_BASIS_VERSION, identity: identity.tag, existingSqlSha256: actualSha, generatedSqlSha256: sha256(generatedSql), schemaSha256: schemaSha, snapshotPath: "drizzle/meta/0030_snapshot.json", snapshotBlobOid: null, snapshotSha256: sha256(snapshotText), snapshotId: targetSnapshot.id, snapshotPrevId: targetSnapshot.prevId, journalPath: "drizzle/meta/_journal.json", journalSha256: sha256(journalText), journalEntry: generatedEntry, journalEntrySha256: canonicalJournalEntrySha256(generatedEntry), authorityState: "VERIFIED_EXISTING_MIGRATION_AUTHORITY", authorityPath: "existing-migration", exactReplayState: "REPLAYABLE", validation: { executionValidated: true, schemaEquivalent: true, dataEquivalent: true, schemaMismatchCount: 0 } }; machine.transition("METADATA_PREPARED");
    const result = { status: "VALIDATED", drizzleKitVersion: DRIZZLE_KIT_VERSION, identity, existingSha256: actualSha, hashBasisVersion: CANONICAL_HASH_BASIS_VERSION, migrationBlobOid: canonicalBlob.oid, generatedSha256: sha256(generatedSql), diagnostic, authority: { schemaEquivalence: authority.schemaEquivalence, dataPreservationEquivalence: authority.dataPreservationEquivalence }, snapshot: { id: targetSnapshot.id, prevId: targetSnapshot.prevId }, journal: { idx: identity.idx, tag: identity.tag }, applied: false, binderStateHistory: machine.history };
