...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s01-u01-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s01_u01.authoring_draft.v1");
  assert.equal(manifest.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(manifest.courseId, "course-isrm");
  assert.equal(manifest.subjectId, "isrm-2025-2027-s01");
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s01-u01");
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-01", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-02", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-27", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S01-U01-O01", "ISRM-S01-U01-O02"]);
  ...
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S01-U01-T01"]);
    assert.ok(objective.questionIds.length > 0, `${objective.id} must link a question`);
  }

  assert.equal(theory.schema, "securium.isrm.s01_u01.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S01-U01-T01");
  ...
  assert.ok(theory.sections.length >= 6, "theory must include the full draft structure");
  ...
  const expectedQuestionIds = ["isrm-q-v2-01", "isrm-q-v2-02", "isrm-q-v2-27"];
  ...
  for (const question of questions.questions) {
    ...
    assert.equal(question.subjectId, "isrm-2025-2027-s01");
    ...
    assert.ok(question.options.some((option) => option.id === question.correctOptionId), `${question.id} answer must be one option`);
  ...
  for (const claim of claims.claims) nonEmpty(claim.claim, `${claim.id}.claim`);
  for (const ref of [...manifest.officialScopeAnchor.claimIds, ...theory.claimIds, ...objectives.objectives.flatMap((objective) => objective.claimIds), ...questions.questions.flatMap((question) => question.claimIds)]) {
