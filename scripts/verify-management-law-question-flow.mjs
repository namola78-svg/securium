...
if (!VALID_TARGETS.has(target)) {
  fail("MANAGEMENT_LAW_QUESTION_FLOW_TARGET_INVALID");
}
...
async function verifyD1Local() {
  ...
  console.log("MANAGEMENT_LAW_QUESTION_FLOW_D1_LOCAL_OK");
}
...
async function verifyPostgres() {
  ...
  const sql = postgres(connectionUrl, {
    ...
    debug: false,
    connection: {
      application_name: "securium-management-law-question-flow-verify",
    },
  });
  ...
  try {
    ...
    fail("MANAGEMENT_LAW_QUESTION_FLOW_POSTGRES_FAILED", safeErrorCode(error));
  } finally {
    await sql.end({ timeout: 5 });
  }

  console.log("MANAGEMENT_LAW_QUESTION_FLOW_POSTGRES_OK");
}

function buildVerificationSql(dialect) {
  const countCast = dialect === "postgres" ? "::int" : "";
  const questionPattern = "management-law-official-sample-q%";
  const courseIds = MANAGEMENT_LAW_COURSE_IDS.map((courseId) =>
    sqlString(courseId),
  ).join(",");
  const contentId = sqlString(MANAGEMENT_LAW_CONTENT_ID);

  return `
WITH management_law_questions AS (
...
course_counts AS (
  ...
  INNER JOIN management_law_questions q ON q.id = qc.question_id
...
content_links AS (
  ...
  INNER JOIN management_law_questions q ON q.id = cql.question_id
...
function verifyRows(rows) {
  ...
  assertCount(
    rowMap,
    `content:${MANAGEMENT_LAW_CONTENT_ID}`,
  ...
  for (const courseId of MANAGEMENT_LAW_COURSE_IDS) {
    ...
    JSON.stringify(
      {
        ...
        courseIds: MANAGEMENT_LAW_COURSE_IDS,
        rows: normalizedRows,
      },
      null,
      2,
...
async function d1Query(configPath, statement) {
  ...
  if (result.code !== 0) {
    fail("MANAGEMENT_LAW_QUESTION_FLOW_D1_FAILED");
  }
...
function parseWranglerResults(stdout) {
  ...
  if (start < 0 || end < start) {
    fail("MANAGEMENT_LAW_QUESTION_FLOW_D1_JSON_MISSING");
  }
  ...
  try {
    ...
    fail("MANAGEMENT_LAW_QUESTION_FLOW_D1_JSON_INVALID");
  }
}
...
function assertCount(rowMap, key, expected) {
  ...
  if (actual !== expected) {
    fail(
      "MANAGEMENT_LAW_QUESTION_FLOW_COUNT_MISMATCH",
