...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s03-u03-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s03_u03.authoring_draft.v1");
  ...
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s03-u03");
  ...
  assert.deepEqual(manifest.continuity.priorUnitIds, ["isrm-2025-2027-s03-u01", "isrm-2025-2027-s03-u02"]);
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-17", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-18", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-30", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S03-U03-O01", "ISRM-S03-U03-O02"]);
  unique(objectiveIds, "objective IDs");
  assert.deepEqual(objectives.objectives[0].questionIds, ["isrm-q-v2-17", "isrm-q-v2-18"]);
  assert.deepEqual(objectives.objectives[1].questionIds, ["isrm-q-v2-30"]);
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S03-U03-T01"]);
  }

  assert.equal(theory.schema, "securium.isrm.s03_u03.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S03-U03-T01");
  ...
  const bridge = theory.sections.find((section) => section.id === "bridge-from-u01-u02");
  assert.ok(bridge.body.includes("S03-U01"), "theory must connect to S03-U01");
  assert.ok(bridge.body.includes("S03-U02"), "theory must connect to S03-U02");

  const questionsById = new Map(questions.questions.map((question) => [question.id, question]));
  const expectedQuestionIds = ["isrm-q-v2-17", "isrm-q-v2-18", "isrm-q-v2-30"];
