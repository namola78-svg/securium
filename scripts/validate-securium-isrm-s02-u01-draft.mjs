...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s02-u01-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s02_u01.authoring_draft.v1");
  assert.equal(manifest.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(manifest.courseId, "course-isrm");
  assert.equal(manifest.subjectId, "isrm-2025-2027-s02");
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s02-u01");
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-07", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-08", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-29", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S02-U01-O01", "ISRM-S02-U01-O02"]);
  ...
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S02-U01-T01"]);
    assert.ok(objective.questionIds.length >= 1, `${objective.id} must link to a question`);
  }

  assert.equal(theory.schema, "securium.isrm.s02_u01.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S02-U01-T01");
  ...
  assert.ok(theory.sections.length >= 8, "theory must include the full draft structure");
  ...
  const bridge = theory.sections.find((section) => section.id === "bridge-from-s01");
  assert.ok(bridge.body.includes("S01"), "theory must connect to S01");
  assert.ok(bridge.body.includes("S01-U02"), "theory must connect to S01-U02");
  assert.ok(bridge.body.includes("S01-U03"), "theory must connect to S01-U03");

  const questionsById = new Map(questions.questions.map((question) => [question.id, question]));
  const expectedQuestionIds = ["isrm-q-v2-07", "isrm-q-v2-08", "isrm-q-v2-29"];
  ...
  for (const question of questions.questions) {
    ...
    assert.equal(question.subjectId, "isrm-2025-2027-s02");
