...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s05-u01-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s05_u01.authoring_draft.v1");
  assert.equal(manifest.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(manifest.courseId, "course-isrm");
  assert.equal(manifest.subjectId, "isrm-2025-2027-s05");
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s05-u01");
  ...
  assert.equal(manifest.continuity.priorOutcomeIds.length, 22);
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-25", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-26", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S05-U01-O01", "ISRM-S05-U01-O02"]);
  unique(objectiveIds, "objective IDs");
  assert.deepEqual(objectives.objectives[0].questionIds, ["isrm-q-v2-25"]);
  assert.deepEqual(objectives.objectives[1].questionIds, ["isrm-q-v2-26"]);
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S05-U01-T01"]);
  }

  assert.equal(theory.schema, "securium.isrm.s05_u01.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S05-U01-T01");
  ...
  const bridge = theory.sections.find((section) => section.id === "bridge-from-subjects-i-to-iv");
  for (const prior of ["Subject I", "Subject II", "Subject III", "Subject IV"]) {
  ...
  const expectedQuestionIds = ["isrm-q-v2-25", "isrm-q-v2-26"];
  ...
  for (const question of questions.questions) {
    ...
    assert.equal(question.subjectId, "isrm-2025-2027-s05");
