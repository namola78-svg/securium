...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s03-u02-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s03_u02.authoring_draft.v1");
  ...
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s03-u02");
  ...
  assert.equal(manifest.continuity.priorUnitId, "isrm-2025-2027-s03-u01");
  assert.deepEqual(manifest.continuity.priorOutcomeIds, ["ISRM-S03-U01-O01", "ISRM-S03-U01-O02"]);
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-15", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-16", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S03-U02-O01", "ISRM-S03-U02-O02"]);
  unique(objectiveIds, "objective IDs");
  assert.deepEqual(objectives.objectives[0].questionIds, ["isrm-q-v2-15"]);
  assert.deepEqual(objectives.objectives[1].questionIds, ["isrm-q-v2-16"]);
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S03-U02-T01"]);
  }

  assert.equal(theory.schema, "securium.isrm.s03_u02.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S03-U02-T01");
  ...
  const expectedQuestionIds = ["isrm-q-v2-15", "isrm-q-v2-16"];
