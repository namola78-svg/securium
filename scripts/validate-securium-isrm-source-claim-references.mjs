...
const USED_IN_FILES = REQUIRED_FILES.slice(0, 4);
...
export async function validateSourceClaimReferences(root) {
  const [manifest, objectives, theory, questions, claims] = await Promise.all(
    REQUIRED_FILES.map((fileName) => readJson(root, fileName)),
  );
  ...
  for (const [fileName, claimIds] of actualClaimIdsByFile) {
    for (const claimId of claimIds) {
      assert.ok(
        claimId.startsWith(expectedClaimPrefix),
        `${root}/${fileName} references claim from another learning unit ${claimId}`,
      );
...
export async function validateAllSourceClaimReferences(roots = ISRM_DRAFT_ROOTS) {
  ...
  for (const root of roots) {
    const packageResult = await validateSourceClaimReferences(root);
    packages.push(packageResult);
    const claims = await readJson(root, "source-claims.json");
    for (const claim of claims.claims) {
      const previousRoot = claimOwners.get(claim.id);
      assert.equal(previousRoot, undefined, `source claim ID ${claim.id} is duplicated across packages: ${previousRoot} and ${root}`);
      claimOwners.set(claim.id, root);
    }
  }
