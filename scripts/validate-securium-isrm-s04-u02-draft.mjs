...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s04-u02-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s04_u02.authoring_draft.v1");
  assert.equal(manifest.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(manifest.courseId, "course-isrm");
  assert.equal(manifest.subjectId, "isrm-2025-2027-s04");
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s04-u02");
  ...
  assert.deepEqual(manifest.officialScopeAnchor.subjectRange, [
    { majorItem: "정보보호 위험 관리 계획", subitems: ["정보보호 관리의 이해", "정보보호 위험 관리 거버넌스", "정보보호 관리체계(생명주기) 수립"] },
    { majorItem: "정보보호 위험 평가", subitems: ["위험관리 평가 방법론 선정 및 준비", "정보보호 위험 평가"] },
  ]);
  assert.deepEqual(manifest.continuity.priorUnitIds, [
  ...
  assert.deepEqual(manifest.continuity.priorOutcomeIds, [
    "ISRM-S03-U01-O01",
    "ISRM-S03-U01-O02",
    "ISRM-S03-U02-O01",
    "ISRM-S03-U02-O02",
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-23", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-24", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S04-U02-O01", "ISRM-S04-U02-O02"]);
  unique(objectiveIds, "objective IDs");
  assert.deepEqual(objectives.objectives[0].questionIds, ["isrm-q-v2-23"]);
  assert.deepEqual(objectives.objectives[1].questionIds, ["isrm-q-v2-24"]);
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S04-U02-T01"]);
  }

  assert.equal(theory.schema, "securium.isrm.s04_u02.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S04-U02-T01");
  ...
  for (const section of theory.sections) {
    ...
    nonEmpty(section.heading, `${section.heading}.heading`);
  ...
  const bridge = theory.sections.find((section) => section.id === "bridge-from-s03-and-s04-u01");
  for (const prior of ["S03-U01", "S03-U02", "S03-U03", "S03-U04", "S04-U01"]) {
    assert.ok(bridge.body.includes(prior), `theory must connect to ${prior}`);
  }

  const questionsById = new Map(questions.questions.map((question) => [question.id, question]));
  const expectedQuestionIds = ["isrm-q-v2-23", "isrm-q-v2-24"];
  ...
  for (const question of questions.questions) {
    ...
    assert.equal(question.subjectId, "isrm-2025-2027-s04");
