...
export const DRAFT_ROOT = resolve("content-drafts/securium-isrm-s01-u03-authoring");
...
export async function validateDraft(root = DRAFT_ROOT) {
  ...
  assert.equal(manifest.schema, "securium.isrm.s01_u03.authoring_draft.v1");
  ...
  assert.equal(manifest.learningUnitId, "isrm-2025-2027-s01-u03");
  ...
  assert.deepEqual(manifest.existingCanonicalAuthorityRows, [
    { id: "isrm-q-v2-05", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
    { id: "isrm-q-v2-06", answerAuthority: "PRESENT", explanation: "PRESENT", decision: "ACCEPT", bodyPayloadPresent: false },
  ]);

  const objectiveIds = objectives.objectives.map((objective) => objective.id);
  assert.deepEqual(objectiveIds, ["ISRM-S01-U03-O01", "ISRM-S01-U03-O02"]);
  ...
  for (const objective of objectives.objectives) {
    ...
    assert.deepEqual(objective.theoryIds, ["ISRM-S01-U03-T01"]);
    assert.equal(objective.questionIds.length, 1);
  }

  assert.equal(theory.schema, "securium.isrm.s01_u03.theory_draft.v1");
  assert.equal(theory.status, "DRAFT_UNPUBLISHED_REVIEW_REQUIRED");
  assert.equal(theory.id, "ISRM-S01-U03-T01");
  ...
  assert.ok(theory.sections.length >= 7, "theory must include the full draft structure");
  ...
  assert.ok(theory.sections.find((section) => section.id === "bridge-from-u02").body.includes("U02"));

  const questionsById = new Map(questions.questions.map((question) => [question.id, question]));
  const expectedQuestionIds = ["isrm-q-v2-05", "isrm-q-v2-06"];
  ...
  for (const objective of objectives.objectives) {
    const linked = questions.questions.find((question) => objective.questionIds.includes(question.id));
    assert.ok(linked, `${objective.id} must link to a question`);
