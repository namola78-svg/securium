...
CREATE TRIGGER `practical_rubric_versions_evaluation_check_insert` BEFORE INSERT ON `practical_rubric_versions` WHEN (NEW.evaluation_semantic_hash IS NOT NULL AND (length(NEW.evaluation_semantic_hash) <> 64 OR NEW.evaluation_semantic_hash GLOB '*[^0-9a-f]*')) OR (NEW.evaluation_method IS NOT NULL AND NEW.evaluation_method NOT IN ('RULE_BASED', 'STRUCTURED_HUMAN_REVIEW', 'HYBRID')) OR (NEW.evidence_classification IS NOT NULL AND NEW.evidence_classification NOT IN ('ELIGIBLE_PERFORMANCE_EVIDENCE', 'ELIGIBLE_AFTER_HUMAN_EVALUATION', 'SUPPORTING_ACTIVITY_ONLY')) BEGIN SELECT RAISE(ABORT, 'practical_rubric_versions_evaluation_check'); END;--> statement-breakpoint
CREATE TRIGGER `practical_rubric_versions_evaluation_check_update` BEFORE UPDATE OF evaluation_semantic_hash, evaluation_method, evidence_classification ON `practical_rubric_versions` WHEN (NEW.evaluation_semantic_hash IS NOT NULL AND (length(NEW.evaluation_semantic_hash) <> 64 OR NEW.evaluation_semantic_hash GLOB '*[^0-9a-f]*')) OR (NEW.evaluation_method IS NOT NULL AND NEW.evaluation_method NOT IN ('RULE_BASED', 'STRUCTURED_HUMAN_REVIEW', 'HYBRID')) OR (NEW.evidence_classification IS NOT NULL AND NEW.evidence_classification NOT IN ('ELIGIBLE_PERFORMANCE_EVIDENCE', 'ELIGIBLE_AFTER_HUMAN_EVALUATION', 'SUPPORTING_ACTIVITY_ONLY')) BEGIN SELECT RAISE(ABORT, 'practical_rubric_versions_evaluation_check'); END;--> statement-breakpoint
